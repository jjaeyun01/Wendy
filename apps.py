...
# Keywords used to filter /Applications for each category
CATEGORY_HINTS = {
...
def filter_apps(all_apps: list[str], category: str) -> list[str]:
    ...
    # if nothing matched, show everything so user is never stuck
    return matched if matched else all_apps


def run_apps(stdscr, color_mode: str) -> None:
    curses.curs_set(0)
    curses.start_color()
    ...
    while True:
        ...
        if key == curses.KEY_UP:
            ...
            while True:
                ...
                if not filtered:
                    safe(3, 4, "No apps found in /Applications.", DIM)
                else:
                    for screen_i, idx in enumerate(range(scroll, min(scroll + list_rows, len(filtered)))):
                        ...
                        if app_cursor == idx:
                            safe(row, 2, ">", RED)
                            safe(row, 4, app_name, RED)
                        else:
                            safe(row, 4, app_name, NORM)
